...
describe VacancyParser do
  ...
  describe '#get_parsed_details' do
    it 'will parse the injected html doc' do
      ...
      vacancy_parser.parse_all_data_in(doc_arr)
    end

    it 'will extract the injected doc\'s job details' do
      expect { vacancy_parser.parse_all_data_in(doc_arr) }.to change { vacancy_parser.send(:vacancies) }.from([]).to([{ nil => 'vacency detail' }])
    end
