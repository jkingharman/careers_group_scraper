module CareersGroupScraper
  # First it iterates over pages of Careers Group's Jobboard, scraping any
  # vacancy links it finds. Then it makes a request to each vacancy and collects
  # the responses.
  class JobScraper
    ...
    def set_listing_url
      @page_count += 1
      @listing_url =
        "#{BASE_URL}Vacancies.aspx?st=#{search_term}&page=#{page_count}"
    end
